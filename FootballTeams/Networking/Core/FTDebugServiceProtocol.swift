//
//  FTDebugServiceProtocol.swift
//  FootballTeams
//
//  Created by Javier Manzo on 09/02/2023.
//

import Foundation

public protocol FTDebugServiceProtocol: AnyObject {
    var debugType: FTDebugServiceType { get set }
}

public enum FTDebugServiceType {
    case none
    case request
    case response
...
public extension FTDebugServiceProtocol {
    ...
    func printRequest(request: URLRequest) {
        if let service = self as? FTServiceProtocolBase,
           self.debugType == .request || self.debugType == .requestAndResponse {
            let curl = self.cURLRepresentation(request: request)
    ...
    private func cURLRepresentation(request: URLRequest) -> String {
